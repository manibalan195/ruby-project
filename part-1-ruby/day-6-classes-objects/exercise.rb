...
def book_summary(books)
  unless books.empty? 
    ...
    puts "Recent books count : #{books.count {|b| b.recent?}}"
  else
...
def add_book(books,book_hash)
  ...
  elsif validate_input( book_hash[:title],"title") && validate_input(book_hash[:author],"author")
    books.push( Book.new( book_hash[:title], book_hash[:author], book_hash[:year]))
    return "Book added successfully..."
  else
    return "Filed cannot be blank."
  end
end
